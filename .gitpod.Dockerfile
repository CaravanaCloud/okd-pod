...
RUN bash -c "mkdir -p '/tmp/openshift-installer' \
    && wget -nv -O '/tmp/openshift-installer/openshift-install-linux.tar.gz' '${URL_INSTALLER}' \
...
ARG URL_CLIENTS="${URL_OKD}/openshift-client-linux-4.12.0-0.okd-2023-04-16-041331.tar.gz"
RUN bash -c "mkdir -p '/tmp/oc' \
    && wget -nv -O '/tmp/oc/openshift-client-linux.tar.gz' '${URL_CLIENTS}' \
